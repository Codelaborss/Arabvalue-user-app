...
class PartnerStoreCard extends StatelessWidget {
                      ...
                      Container(
                        ...
                        child: ClipOval(
                          child: CustomImage(
                            ...
                            height: isRecommended ? 75 : 85,
                            width: isRecommended ? 75 : 85,
                            fit: BoxFit.cover,
                          ),
                        ),
                      ),
                  ...
                  Row(
                    children: [
                      Icon(Icons.location_on,
                          size: isRecommended ? 16 : 18, color: Colors.black87),
                      ...
                      _buildBadge(
                        ...
                        Get.find<StoreController>().isOpenNow(store)
                            ? 'open_now'.tr
                            : 'closed_now'.tr,
                        iconColor: Get.find<StoreController>().isOpenNow(store)
                            ? Colors.green
                            : Colors.red,
                        textColor: Get.find<StoreController>().isOpenNow(store)
                            ? Colors.green
                            : Colors.red,
                        isRecommended: isRecommended,
                      ),
  ...
  Widget _buildBadge(BuildContext context, IconData icon, String text,
      {Color? iconColor, Color? textColor, bool isRecommended = false}) {
    return Container(
      padding: EdgeInsets.symmetric(
          horizontal: isRecommended ? 10 : 12, vertical: isRecommended ? 5 : 7),
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: [
          Icon(icon,
              size: isRecommended
                  ? (icon == Icons.circle ? 10 : 16)
                  : (icon == Icons.circle ? 12 : 20),
              color: iconColor ?? Colors.black87),
          const SizedBox(width: 4),
          Text(text,
              style: robotoBlack.copyWith(
                  fontSize: isRecommended ? 11 : 13,
