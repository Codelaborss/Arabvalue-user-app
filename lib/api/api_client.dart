...
class ApiClient extends GetxService {
  ...
  Future<int?> getUserId() async {
    try {
      String? fullLoginResponse =
          sharedPreferences.getString(AppConstants.fullLoginResponse);
      if (fullLoginResponse != null) {
        Map<String, dynamic> loginData = jsonDecode(fullLoginResponse);
        if (loginData.containsKey('is_exist_user') &&
            loginData['is_exist_user'] != null) {
          Map<String, dynamic> userData = loginData['is_exist_user'];
          int? userId = userData['id'] as int?;
          if (kDebugMode) {
            print('====> Retrieved User ID from login response: $userId');
          }
          return userId;
        }
      }
  ...
  Future<Map<String, String>> getHeaderWithUserId() async {
    Map<String, String> headerWithUserId = Map.from(_mainHeaders);
    int? userId = await getUserId();
    if (userId != null) {
      headerWithUserId['userId'] = userId.toString();
      if (kDebugMode) {
        print('====> Added userId to headers: $userId');
        print('====> Final headers keys: ${headerWithUserId.keys.toList()}');
      }
    } else {
      if (kDebugMode) {
        print('====> No userId found, headers remain unchanged');
      }
    }
    return headerWithUserId;
  }
  ...
  Future<Response> getData(String uri,
    ...
    try {
      Map<String, String> finalHeaders = headers ?? _mainHeaders;
      if (kDebugMode) {
        print('====> API Call: $uri');
        print('====> Header keys: ${finalHeaders.keys.toList()}');
        print('====> Header userId: ${finalHeaders['userId']}');
        print('====> Full Header count: ${finalHeaders.length}');
      }
