...
class CheckoutController extends GetxController implements GetxService {
  ...
  void callback(
    ...
    if (isSuccess) {
      ...
      try {
        final cartItems = Get.find<CartController>().cartList;
        debugPrint(
            '--- Checklist Callback: Detecting Vouchers (Cart size: ${cartItems.length}) ---');
        for (final c in cartItems) {
          String type = (c.item?.type ?? '').toLowerCase();
          String vType = (c.item?.voucherType ?? '').toLowerCase();
          String bundle = (c.item?.bundleType ?? '').toLowerCase();
          String ids = (c.item?.voucherIds ?? '').toLowerCase();

          if (type == 'voucher' ||
              vType.isNotEmpty ||
              bundle.contains('gift') ||
              ids.contains('flat')) {
            isVoucher = true;
            if (vType.contains('flat') || ids.contains('flat')) {
              isFlat = true;
            }
            discountAmount = c.item?.discount ?? 0.0;
            debugPrint(
                'Voucher Detected: isVoucher=$isVoucher, isFlat=$isFlat, discount=$discountAmount');
            break;
          }
        }
      } catch (e) {
        debugPrint('Error detecting voucher in callback: $e');
      }
      // ─────────────────────────────────────────────────────────────────────
      ...
      if (paymentMethodIndex == 2) {
        if (GetPlatform.isWeb && digitalPaymentName != 'cybersource') {
          ...
          selectedUrl =
              ...
              '&payment_method=$digitalPaymentName&payment_platform=web&amount=$amount&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
  ...
  Future<void> getSurgePrice(
    ...
    SurgePriceModel? surgePriceModel =
