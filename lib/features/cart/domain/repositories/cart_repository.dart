  ...
  Future<List<OnlineCartModel>?> _addToCartOnline(OnlineCart cart) async {
    ...
    if (response.statusCode == 200) {
      if (response.body is List) {
        ...
        if (response.body is Map && response.body['status'] == 'rejected') {
          int? cartId = response.body['cart_id'];
          if (cartId != null) {
            await _removeCartItemOnline(cartId);
          }
          return null;
        }
        onlineCartList = await _getCartDataOnline();
      }
    } else if (response.statusCode == 1 && cart.timeout != null) {
      // Handle timeout (likely Flat Voucher waiting for vendor)
      print(
          '=====> Add to cart timed out after ${cart.timeout} seconds. Cleaning up...');
      List<OnlineCartModel>? currentCart = await _getCartDataOnline();
      if (currentCart != null) {
        for (OnlineCartModel item in currentCart) {
          if (item.cartGroupId == cart.cartGroup) {
            print('=====> Removing orphaned cart item: ${item.id}');
            await _removeCartItemOnline(item.id!);
          }
        }
      }
    }
