...
class AuthRepository implements AuthRepositoryInterface {
  ...
  @override
  Future<Response> loginWithPin({required String pinCode}) async {
  ...
  Future<bool> clearSharedData({bool removeToken = true}) async {
    ...
    // sharedPreferences.remove(AppConstants.clientData);
    // sharedPreferences.remove(AppConstants.clientAppData);
    // print('====> 🗑️ Logo & Client Data Cleared on Logout');
  ...
  @override
  Future<void> saveFullLoginResponse(String responseData) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString(AppConstants.fullLoginResponse, responseData);
    print('====> Full Login Response Saved ✅');
  }

  @override
  Future<String> getFullLoginResponse() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString(AppConstants.fullLoginResponse) ?? '';
  }

  @override
  Future<void> saveClientData(String clientData) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString(AppConstants.clientData, clientData);
    print('====> Client Data Saved ✅');
  }

  @override
  Future<String> getClientData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString(AppConstants.clientData) ?? '';
  }

  @override
  Future<void> saveClientAppData(String appData) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString(AppConstants.clientAppData, appData);
    print('====> Client App Data Saved ✅');
  }

  @override
  Future<String> getClientAppData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString(AppConstants.clientAppData) ?? '';
  }

  @override
  Future<void> saveAppThemes(String themesJson) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString(AppConstants.appThemes, themesJson);
    print('====> App Themes Saved ✅');
  }

  @override
  Future<String> getAppThemes() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString(AppConstants.appThemes) ?? '';
  }

  @override
  Future<void> clearAppThemes() async {
    await sharedPreferences.remove(AppConstants.appThemes);
  }
  ...
  Future<String> getAppLogo() async {
    try {
      String appDataJson =
          sharedPreferences.getString(AppConstants.clientAppData) ?? '';

      if (appDataJson.isEmpty) {
        print('====> ⚠️ No app data found');
        return '';
      }

      Map<String, dynamic> appData = jsonDecode(appDataJson);

      String appLogo = appData['app_logo'] ?? '';

      print('====> 🖼️ App Logo: $appLogo');
      return appLogo;
    } catch (e) {
  ...
  Future<String> getAppLogoUrl() async {
    try {
      String appLogo = await getAppLogo();

      if (appLogo.isEmpty) {
        print('====> ⚠️ No app logo path found');
        return '';
      }

      String fullUrl = '${AppConstants.baseUrl}/public/$appLogo';

      print('====> 🖼️ Full App Logo URL: $fullUrl');
      return fullUrl;
    } catch (e) {
      print('====> ❌ Error getting app logo URL: $e');
      return '';
    }
  }

  @override
  Future<void> saveClientAppThemes(String themesJson) async {
    await sharedPreferences.setString(AppConstants.clientApiThemes, themesJson);
  }

  @override
  Future<String> getClientAppThemes() async {
    return sharedPreferences.getString(AppConstants.clientApiThemes) ?? '';
  }

  @override
  Future<void> clearClientAppThemes() async {
    await sharedPreferences.remove(AppConstants.clientApiThemes);
  }

  @override
  Future<void> clearClientAppData() async {
    await sharedPreferences.remove(AppConstants.clientAppData);
  }
}
