...
class AuthService implements AuthServiceInterface {
  ...
  Future<ResponseModel> loginWithPin(
      {required String pinCode, bool alreadyInApp = false}) async {
    Response response =
        await authRepositoryInterface.loginWithPin(pinCode: pinCode);
    if (response.statusCode == 200) {
      AuthResponseModel authResponse =
          AuthResponseModel.fromJson(response.body);
      await _updateHeaderFunctionality(authResponse,
  ...
  Future<void> _updateHeaderFunctionality(AuthResponseModel authResponse,
    ...
    if (authResponse.token != null && authResponse.token!.isNotEmpty) {
      authRepositoryInterface.saveUserToken(authResponse.token ?? '',
          alreadyInApp: alreadyInApp);
  ...
  Future<ResponseModel> loginWithSocialMedia(SocialLogInBody socialLogInModel,
    ...
    Response response =
        await authRepositoryInterface.loginWithSocialMedia(socialLogInModel);
    if (response.statusCode == 200) {
      AuthResponseModel authResponse =
          AuthResponseModel.fromJson(response.body);
      await _updateHeaderFunctionality(authResponse);
  ...
  Future<void> saveFullLoginResponse(String responseData) async {
    await authRepositoryInterface.saveFullLoginResponse(responseData);
  }

  @override
  Future<String> getFullLoginResponse() async {
    return authRepositoryInterface.getFullLoginResponse();
  }

  @override
  Future<void> saveClientData(String clientData) async {
    await authRepositoryInterface.saveClientData(clientData);
  }

  @override
  Future<String> getClientData() async {
    return authRepositoryInterface.getClientData();
  }

  @override
  Future<void> saveClientAppData(String appData) async {
    await authRepositoryInterface.saveClientAppData(appData);
  }

  @override
  Future<String> getClientAppData() async {
    return authRepositoryInterface.getClientAppData();
  }

  @override
  Future<void> saveAppThemes(String themesJson) async {
    await authRepositoryInterface.saveAppThemes(themesJson);
  }

  @override
  Future<String> getAppThemes() async {
    return await authRepositoryInterface.getAppThemes();
  }
  ...
  Future<String> getAppLogo() async {
    return await authRepositoryInterface.getAppLogo();
  }

  @override
  Future<String> getAppLogoUrl() async {
    return await authRepositoryInterface.getAppLogoUrl();
  }

  @override
  Future<void> saveClientAppThemes(String themesJson) async {
    await authRepositoryInterface.saveClientAppThemes(themesJson);
  }

  @override
  Future<String> getClientAppThemes() async {
    return await authRepositoryInterface.getClientAppThemes();
  }
