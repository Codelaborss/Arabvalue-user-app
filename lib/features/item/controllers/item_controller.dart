...
class ItemController extends GetxController implements GetxService {
  ...
  List<Item>? _hotVoucherItemList;
  List<Item>? get hotVoucherItemList => _hotVoucherItemList;

  List<Item>? _newVoucherItemList;
  List<Item>? get newVoucherItemList => _newVoucherItemList;
  ...
  List<int> _productSelect = [];
  List<int> get productSelect => _productSelect;

  List<int> _voucherVariationIndex = [];
  List<int> get voucherVariationIndex => _voucherVariationIndex;
  ...
  int? _selectedBogoProductAIndex;
  int? get selectedBogoProductAIndex => _selectedBogoProductAIndex;

  int? _selectedBogoProductBIndex;
  int? get selectedBogoProductBIndex => _selectedBogoProductBIndex;

  List<List<bool?>> _selectedBogoVariationsA = [];
  List<List<bool?>> get selectedBogoVariationsA => _selectedBogoVariationsA;

  List<List<bool?>> _selectedBogoVariationsB = [];
  List<List<bool?>> get selectedBogoVariationsB => _selectedBogoVariationsB;

  List<bool> _selectedBogoAddonsA = [];
  List<bool> get selectedBogoAddonsA => _selectedBogoAddonsA;

  List<bool> _selectedBogoAddonsB = [];
  List<bool> get selectedBogoAddonsB => _selectedBogoAddonsB;

  List<int?> _selectedBogoAddonQtyA = [];
  List<int?> get selectedBogoAddonQtyA => _selectedBogoAddonQtyA;

  List<int?> _selectedBogoAddonQtyB = [];
  List<int?> get selectedBogoAddonQtyB => _selectedBogoAddonQtyB;

  // Mix & Match selection state
  List<MixMatchSelection> _selectedMixMatchProducts = [];
  List<MixMatchSelection> get selectedMixMatchProducts =>
      _selectedMixMatchProducts;

  int? _currentMixMatchProductIndex;
  int? get currentMixMatchProductIndex => _currentMixMatchProductIndex;

  List<List<bool?>> _currentMixMatchVariations = [];
  List<List<bool?>> get currentMixMatchVariations => _currentMixMatchVariations;

  List<bool> _currentMixMatchAddons = [];
  List<bool> get currentMixMatchAddons => _currentMixMatchAddons;

  List<int?> _currentMixMatchAddonQty = [];
  List<int?> get currentMixMatchAddonQty => _currentMixMatchAddonQty;
  ...
  void clearItemLists() {
    ...
    _hotVoucherItemList = null;
    _newVoucherItemList = null;
  }
  ...
  Future<void> getHotVoucherList(bool reload, bool notify,
      {DataSourceEnum dataSource = DataSourceEnum.local,
      bool fromRecall = false}) async {
    if (reload) {
      _hotVoucherItemList = null;
    ...
    if (_hotVoucherItemList == null || reload || fromRecall) {
      List<Item>? items;
      if (dataSource == DataSourceEnum.local) {
        items = await itemServiceInterface.getHotVoucherList(dataSource);
        _prepareHotVoucherItems(items);

        getHotVoucherList(false, notify,
            dataSource: DataSourceEnum.client, fromRecall: true);
      } else {
        items = await itemServiceInterface.getHotVoucherList(dataSource);
        _prepareHotVoucherItems(items);
  ...
  _prepareHotVoucherItems(List<Item>? items) {
    if (items != null) {
      _hotVoucherItemList = [];
      _hotVoucherItemList!.addAll(items);
      _isLoading = false;
    }
    update();
  }

  Future<void> getNewVoucherList(bool reload, bool notify,
      {DataSourceEnum dataSource = DataSourceEnum.local,
      bool fromRecall = false}) async {
    if (reload) {
      _newVoucherItemList = null;
    ...
    if (_newVoucherItemList == null || reload || fromRecall) {
      List<Item>? items;
      if (dataSource == DataSourceEnum.local) {
        items = await itemServiceInterface.getNewVoucherList(dataSource);
        _prepareNewVoucherItems(items);

        getNewVoucherList(false, notify,
            dataSource: DataSourceEnum.client, fromRecall: true);
      } else {
        items = await itemServiceInterface.getNewVoucherList(dataSource);
        _prepareNewVoucherItems(items);
  ...
  _prepareNewVoucherItems(List<Item>? items) {
    if (items != null) {
      _newVoucherItemList = [];
      _newVoucherItemList!.addAll(items);
      _isLoading = false;
    }
    update();
  }
  ...
  void initData(Item? item, CartModel? cart) {
    ...
    _collapseVariation = [];
    _productSelect = [];
    _voucherVariationIndex = [];
    if (item?.product != null) {
      for (var p in item!.product!) {
        if (p.foodVariations != null) {
          for (var _ in p.foodVariations!) {
            _voucherVariationIndex.add(-1);
          }
        }
      }
    }

    _selectedBogoProductAIndex = null;
    _selectedBogoProductBIndex = null;
    _selectedBogoVariationsA = [];
    _selectedBogoVariationsB = [];
    _selectedBogoAddonsA = [];
    _selectedBogoAddonsB = [];
    _selectedBogoAddonQtyA = [];
    _selectedBogoAddonQtyB = [];
    ...
    if (cart != null) {
      ...
      // Always initialize selectedVariations if foodVariations exist
      if (item?.foodVariations != null && item!.foodVariations!.isNotEmpty) {
      ...
      } else if (ModuleHelper.getModuleConfig(item?.moduleType).newVariation ??
          false) {
        _selectedVariations.addAll(itemServiceInterface
            .initializeSelectedVariation(item?.foodVariations));
        _collapseVariation.addAll(itemServiceInterface
        ...
        _variationIndex = itemServiceInterface
            .initializeVariationIndexes(item?.choiceOptions);
      }
      _quantity = 1;
      _addOnActiveList
  ...
  void setSelect(int select, bool notify, {bool isToggle = false}) {
    if (isToggle) {
      if (_productSelect.contains(select)) {
        _productSelect.remove(select);
      } else {
        _productSelect.add(select);
      }
    } else {
  ...
  void setVoucherVariation(int variationIndex, int selectedIndex) {
    if (_voucherVariationIndex[variationIndex] == selectedIndex) {
      _voucherVariationIndex[variationIndex] = -1;
    } else {
      _voucherVariationIndex[variationIndex] = selectedIndex;
    }
    update();
  }

  void selectBogoProductA(int? index) {
    if (_selectedBogoProductAIndex == index) {
      _selectedBogoProductAIndex = null;
      _selectedBogoVariationsA = [];
      _selectedBogoAddonsA = [];
      _selectedBogoAddonQtyA = [];
    } else {
      _selectedBogoProductAIndex = index;
      Item product = _item!.product![index!];
      _selectedBogoVariationsA = itemServiceInterface
          .initializeSelectedVariation(product.foodVariations);
      _selectedBogoAddonsA =
          itemServiceInterface.initializeAddonActiveList(product.addOns);
      _selectedBogoAddonQtyA =
          itemServiceInterface.initializeAddonQtyList(product.addOns);
    }
    update();
  }

  void initBogoProductA(int index) {
    Item product = _item!.product![index];
    _selectedBogoVariationsA = itemServiceInterface
        .initializeSelectedVariation(product.foodVariations);
    _selectedBogoAddonsA =
  ...
  void confirmBogoProductA(int index) {
    _selectedBogoProductAIndex = index;
    update();
  }

  void selectBogoProductB(int? index) {
    if (_selectedBogoProductBIndex == index) {
      _selectedBogoProductBIndex = null;
      _selectedBogoVariationsB = [];
      _selectedBogoAddonsB = [];
      _selectedBogoAddonQtyB = [];
    } else {
      _selectedBogoProductBIndex = index;
      Item product = _item!.productB![index!];
      _selectedBogoVariationsB = itemServiceInterface
          .initializeSelectedVariation(product.foodVariations);
      _selectedBogoAddonsB =
          itemServiceInterface.initializeAddonActiveList(product.addOns);
      _selectedBogoAddonQtyB =
  ...
  void initBogoProductB(int index) {
    Item product = _item!.productB![index];
    _selectedBogoVariationsB = itemServiceInterface
        .initializeSelectedVariation(product.foodVariations);
    _selectedBogoAddonsB =
  ...
  void confirmBogoProductB(int index) {
    _selectedBogoProductBIndex = index;
    update();
  }

  void setBogoVariationIndex(int index, int i, Item product, bool isProductA) {
    if (isProductA) {
      if (_selectedBogoVariationsA[index][i] ?? false) {
        _selectedBogoVariationsA[index][i] = false;
      } else {
        _selectedBogoVariationsA =
            itemServiceInterface.setNewCartVariationIndex(
                index, i, product.foodVariations!, _selectedBogoVariationsA);
      }
    } else {
      if (_selectedBogoVariationsB[index][i] ?? false) {
        _selectedBogoVariationsB[index][i] = false;
      } else {
        _selectedBogoVariationsB =
            itemServiceInterface.setNewCartVariationIndex(
                index, i, product.foodVariations!, _selectedBogoVariationsB);
      }
    }
    update();
  }

  void addBogoAddOn(bool isAdd, int index, bool isProductA) {
    if (isProductA) {
      _selectedBogoAddonsA[index] = isAdd;
    } else {
      _selectedBogoAddonsB[index] = isAdd;
    }
    update();
  }

  void setBogoAddOnQuantity(bool isIncrement, int index, bool isProductA) {
    if (isProductA) {
      _selectedBogoAddonQtyA[index] = itemServiceInterface.setAddOnQuantity(
          isIncrement, _selectedBogoAddonQtyA[index]!);
    } else {
      _selectedBogoAddonQtyB[index] = itemServiceInterface.setAddOnQuantity(
          isIncrement, _selectedBogoAddonQtyB[index]!);
    }
    update();
  }

  void setMixMatchVariationIndex(int index, int i, Item product) {
    if (_currentMixMatchVariations[index][i] ?? false) {
      _currentMixMatchVariations[index][i] = false;
    } else {
      _currentMixMatchVariations =
          itemServiceInterface.setNewCartVariationIndex(
              index, i, product.foodVariations!, _currentMixMatchVariations);
    }
    update();
  }

  void addMixMatchAddOn(bool isAdd, int index) {
    _currentMixMatchAddons[index] = isAdd;
    update();
  }

  void setMixMatchAddOnQuantity(bool isIncrement, int index) {
    _currentMixMatchAddonQty[index] = itemServiceInterface.setAddOnQuantity(
        isIncrement, _currentMixMatchAddonQty[index]!);
    update();
  }

  void initMixMatch(Item? item) {
    _selectedMixMatchProducts = [];
    _currentMixMatchProductIndex = null;
    clearCurrentMixMatchData();
    // update(); // Avoid unnecessary update in init
  }

  void clearCurrentMixMatchData() {
    _currentMixMatchVariations = [];
    _currentMixMatchAddons = [];
    _currentMixMatchAddonQty = [];
  }

  void selectMixMatchProduct(int index) {
    _currentMixMatchProductIndex = index;
    Item product = _item!.product![index];

    _currentMixMatchVariations = itemServiceInterface
        .initializeSelectedVariation(product.foodVariations);
  ...
  void collapseMixMatchProduct() {
    _currentMixMatchProductIndex = null;
    clearCurrentMixMatchData();
    update();
  }

  void confirmMixMatchProduct(Item product, double price, double addonsPrice) {
    List<int> addonIds = [];
    List<int> addonQtys = [];

    if (product.addOns != null) {
      for (int i = 0; i < product.addOns!.length; i++) {
        if (_currentMixMatchAddons[i]) {
          addonIds.add(product.addOns![i].id!);
          addonQtys.add(_currentMixMatchAddonQty[i]!);
        }
      }
    }

    List<List<bool?>> variations = [];
    for (var list in _currentMixMatchVariations) {
      variations.add(List.from(list));
    }

    // Check if product already exists in selection and update it
    int existingIndex = -1;
    for (int i = 0; i < _selectedMixMatchProducts.length; i++) {
      if (_selectedMixMatchProducts[i].productIndex ==
          _currentMixMatchProductIndex) {
        existingIndex = i;
        break;
      }
    }

    MixMatchSelection selection = MixMatchSelection(
      productIndex: _currentMixMatchProductIndex!,
      selectedAddonIds: addonIds,
      selectedAddonQuantities: addonQtys,
    ...
    if (existingIndex != -1) {
      _selectedMixMatchProducts[existingIndex] = selection;
    } else {
      if (_selectedMixMatchProducts.length >= (_item!.requiredQuantity ?? 0)) {
        showCustomSnackBar(
            '${'you_can_only_select'.tr} ${'${_item!.requiredQuantity} ${'items'.tr}'}',
            getXSnackBar: true);
        return;
      }
      _selectedMixMatchProducts.add(selection);
    }
  ...
  void updateMixMatchProduct(Item product, double price, double addonsPrice) {
    List<int> addonIds = [];
    List<int> addonQtys = [];
  ...
  void removeMixMatchProduct(int index) {
    _selectedMixMatchProducts.removeAt(index);
    update();
  }

  double getMixMatchTotalAmount(
      double voucherPriceWithDiscount, double? discount, String? discountType) {
    double totalVariations = 0;
    double totalAddons = 0;
    for (var selection in _selectedMixMatchProducts) {
      double variationPrice = selection.totalPrice;
      totalVariations += variationPrice;
      totalAddons += selection.addonsPrice;
    }

    double discountedVariations = PriceConverter.convertWithDiscount(
            totalVariations, discount, discountType) ??
        totalVariations;

    return (voucherPriceWithDiscount + discountedVariations + totalAddons) *
        (_quantity ?? 1);
  }

  double getMixMatchRawTotalAmount(double voucherRawPrice) {
    double totalExtras = 0;
    for (var selection in _selectedMixMatchProducts) {
      double variationPrice = selection.totalPrice;
      totalExtras += variationPrice + selection.addonsPrice;
    }
    return (voucherRawPrice + totalExtras) * (_quantity ?? 1);
  }

  bool isMixMatchProductSelected(int index) {
    for (var selection in _selectedMixMatchProducts) {
      if (selection.productIndex == index) {
        return true;
      }
    }
    return false;
