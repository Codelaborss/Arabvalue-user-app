...
class ItemRepository implements ItemRepositoryInterface {
  ...
  Future getList({
    ...
    if (isHotVoucher) {
      return await _getHotVoucherList(source: source!);
    } else if (isNewVoucher) {
      return await _getNewVoucherList(source: source!);
      ...
      return await _getDiscountedItemList(
          ...
          minPrice: minPrice,
          maxPrice: maxPrice);
    } else if (isHotVoucher) {
      return await _getHotVoucherList(source: source ?? DataSourceEnum.client);
    }
  }
    ...
    switch (source) {
      case DataSourceEnum.client:
        ...
        if (response.statusCode == 200) {
          ...
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body), headers);
        }
  ...
  Future<List<Item>?> _getHotVoucherList(
      {required DataSourceEnum source}) async {
    List<Item>? hotVoucherList;
    String cacheId =
        '${AppConstants.hotVoucherUri}?limit=30&offset=1${Get.find<SplashController>().module!.id!}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient
            .getData('${AppConstants.hotVoucherUri}?limit=30&offset=1');
        if (response.statusCode == 200) {
          hotVoucherList = [];
          hotVoucherList.addAll(ItemModel.fromJson(response.body).items!);
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body), apiClient.getHeader());
        }
  ...
  Future<List<Item>?> _getNewVoucherList(
      {required DataSourceEnum source}) async {
    List<Item>? newVoucherList;
    String cacheId =
        '${AppConstants.newVoucherUri}?limit=30&offset=1${Get.find<SplashController>().module!.id!}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient
            .getData('${AppConstants.newVoucherUri}?limit=30&offset=1');
        if (response.statusCode == 200) {
          newVoucherList = [];
          newVoucherList.addAll(ItemModel.fromJson(response.body).items!);
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body), apiClient.getHeader());
        }
